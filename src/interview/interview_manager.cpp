...
struct InterviewRecord {
    ...
    std::string category;   // 题目类别
    std::string level;      // 难度等级
    std::string answer;     // 候选人回答
    std::string timestamp;  // 回答时间
    int score;              // 分数
    std::string feedback;   // 评语
...
public:
    ...
    std::vector<InterviewRecord> records;      // 面试记录列表
    int max_questions;                         // 最大题目数
    int total_score;                           // 累计总分
    std::time_t start_time;                    // 面试开始时间

    std::string resume_text;                   // 简历文本内容
    nlohmann::json llm_questions;              // LLM生成的问题集
    int current_question_index;                // 当前题目索引

    std::unique_ptr<services::LLMClient> llm_client; // LLM客户端

    std::string followup_question;             // 待追问问题
    std::string summary_text;                  // 面试总结缓存
    bool has_followed_up;                      // 当前题是否已追问过
...
void InterviewSession::GenerateDefaultQuestions(int min_questions) {
    try {
        LOG_INFO(
            "Generating {} default C++ interview questions using LLM...",
            min_questions
        );
        ...
        LOG_INFO(
            "Generated {} default C++ questions",
            pimpl_->max_questions
        );
...
std::string InterviewSession::GetIntroPrompt() const {
    ...
    return R"(现在开始C++技术面试。
    请你作为一位专业且友好的面试官，
    用简短温和的语气向候选人问好（比如'你好，欢迎参加今天的面试'），
    然后礼貌地邀请候选人做一个简单的自我介绍。
    记住要简短，不超过3句话。
    后续对应于面试者的回答都只回复(好的，我们继续)。)";
    // 发的是 CHAT_TEXT_QUERY，服务端会先做 LLM 生成，再把生成结果转 TTS
}
...
std::string InterviewSession::GetNextQuestion() {
    if (!pimpl_->llm_questions.empty()) {
        ...
        pimpl_->has_followed_up = false;  // 进入新主问题时，重置“本题已追问”标记
...
std::string InterviewSession::GetFollowUpQuestion() {
    if (!pimpl_->followup_question.empty()) {
        std::string f_question = pimpl_->followup_question;
        pimpl_->followup_question.clear();
        pimpl_->has_followed_up = true;
        return "追问：" + f_question;
    }
    return "";
}
...
std::string InterviewSession::GenerateSummary() const {
    ...
    // 使用LLM生成总结
    try {
        LOG_INFO("Generating summary using LLM...");
...
nlohmann::json InterviewSession::GenerateReport() const {
    ...
    if (pimpl_->records.empty()) {
        report["error"] = "No interview records";
        return report;
    }
    ...
    report["total_questions"] = pimpl_->max_questions;
    report["average_score"] = avg_score;
    report["total_score"] = pimpl_->total_score;
    report["records"] = nlohmann::json::array();

    // 添加面试记录
    for (const auto& record : pimpl_->records) {
        report["records"].push_back(record.ToJson());
    }
...
std::string InterviewSession::SaveReport(const std::string& filename) const {
    ...
    // 如果调用 SaveReport() 时没有传文件名 , 则自动生成文件名
    ...
    auto report_json = GenerateReport();

    std::ofstream ofs(output_file);
    if (!ofs) {
        throw std::runtime_error("Failed to open file: " + output_file);
    }
    ofs << report_json.dump(2); // dump 2 表示缩进2个空格
    ofs.close();
