...
`JMaa32` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/JMaa32/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/JMaa32/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee JMaa32` to
...
python ./.trellis/scripts/task.py archive 00-join-jmaa32
```
