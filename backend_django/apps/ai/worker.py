...
def refresh_worker_db_connections(*, force: bool = False) -> None:
    """Refresh DB handles for long-running workers before polling.

    Django's request lifecycle normally calls close_old_connections(), but this
    command runs forever outside that lifecycle. With SQLite, a long-lived
    worker can stay alive while missing writes from the web process. Force
    closing at batch boundaries is cheap locally and makes each poll use a
    fresh view of pending tasks.
    """
    if force:
...
def run_ai_task_batch(
    *,
    ...
    refresh_worker_db_connections(force=True)
